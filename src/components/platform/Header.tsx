...
import {
  Bell, Check, ChevronDown, CornerDownLeft, Loader2, LogOut, Search,
} from 'lucide-react';
import { useSession } from './SessionProvider';
import { search, notifications } from '@/lib/core/client';
import GlobalNavMenu from '../layout/GlobalNavMenu';
    ...
    <header className="sticky top-0 z-30 h-16 border-b border-white/5 bg-black/80 backdrop-blur-xl">
      <div className="flex items-center justify-between h-full px-4 lg:px-6 gap-3">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          {/* Global 3-dot menu (ALL navigation lives here now) */}
          <GlobalNavMenu />
          <button
        ...
        <div className="flex items-center gap-2">
              ...
              <div className="absolute right-0 mt-2 w-56 rounded-xl border border-white/10 bg-[#11111a] shadow-2xl py-1.5">
                {me?.workspaces.map((w) => (
                  ...
                  >
                    ...
                    {w.id === me.activeWorkspaceId && <Check className="w-4 h-4 text-[var(--c-accent-text)]" />}
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="relative">
            <button onClick={openNotifs} className="relative p-2 rounded-lg text-gray-400 hover:bg-white/5 hover:text-white transition-colors">
              ...
              {me?.unread ? (
                <span className="absolute top-1.5 right-1.5 min-w-[14px] h-[14px] px-0.5 rounded-full bg-[var(--c-accent)] text-[9px] font-bold text-white flex items-center justify-center">
                  {me.unread}
                </span>
              ) : null}
            </button>
            {showNotifs && (
              <div className="absolute right-0 mt-2 w-80 rounded-xl border border-white/10 bg-[#11111a] shadow-2xl overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 border-b border-white/5">
                  ...
                  <button onClick={markAll} className="text-[11px] text-[var(--c-accent-text)] hover:opacity-80">
                    Mark all read
                  </button>
                </div>
                <div className="max-h-80 overflow-y-auto">
                  {notifLoading && <div className="py-6 text-center"><Loader2 className="w-4 h-4 animate-spin text-[var(--c-accent-text)] mx-auto" /></div>}
                  ...
                  {notifs.map((n) => (
                    ...
                    >
                      <span className={`mt-1 w-1.5 h-1.5 rounded-full shrink-0 ${n.read ? 'bg-gray-700' : 'bg-[var(--c-accent)]'}`} />
          ...
          <div className="relative">
            <button
              ...
              className="w-9 h-9 rounded-full bg-[var(--c-accent)] flex items-center justify-center text-sm font-semibold text-white hover:ring-2 hover:ring-[var(--c-accent-border)] transition-all"
            >
            ...
            {showUser && (
              <div className="absolute right-0 mt-2 w-56 rounded-xl border border-white/10 bg-[#11111a] shadow-2xl py-1.5">
                <div className="px-3 py-2 border-b border-white/5">
                  ...
                  <span className="mt-1 inline-flex uppercase text-[10px] tracking-wider text-[var(--c-accent-text)]">{me?.user.role}</span>
                </div>
        ...
        <div className="fixed inset-0 z-50" onClick={() => setShowPalette(false)}>
          ...
          >
            <div className="flex items-center gap-2 px-4 py-3 border-b border-white/5">
              ...
              />
              {searching && <Loader2 className="w-4 h-4 animate-spin text-[var(--c-accent-text)]" />}
            </div>
            <div className="max-h-80 overflow-y-auto py-1">
              ...
              {hits.map((h) => (
                ...
                >
                  <span className="w-16 shrink-0 text-[10px] uppercase tracking-wider text-[var(--c-accent-text)]">{h.type}</span>
                  <span className="min-w-0">
